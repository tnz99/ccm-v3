...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body style="background-image: url('{{ $backgroundImageName }}')" class="{{ $blur ? 'backdrop-blur-sm' : '' }} font-inter w-full h-full flex flex-col bg-center bg-no-repeat xl:bg-cover xl:bg-left-top overflow-y-hidden xl:overflow-hidden px-10 xl:px-28">
        @if (session('success'))
        <div id="toast-success" class="fixed mt-8 flex items-center w-full max-w-xs p-4 mb-4 text-gray-500 bg-green-100 rounded-xl: shadow dark:text-gray-400 dark:bg-gray-800" role="alert">
            ...
            <button type="button" class="ms-auto -mx-1.5 -my-1.5 bg-green-100 text-gray-400 hover:text-gray-900 rounded-xl: focus:ring-2 focus:ring-gray-300 p-1.5 hover:bg-gray-100 inline-flex items-center justify-center h-8 w-8 dark:text-gray-500 dark:hover:text-white dark:bg-gray-800 dark:hover:bg-gray-700" data-dismiss-target="#toast-success" aria-label="Close">
        ...
        @if (session('error'))
        <div id="toast-danger" class="fixed mt-8 flex items-center w-full max-w-xs p-4 mb-4 text-gray-500 bg-red-100 rounded-xl: shadow dark:text-gray-400 dark:bg-gray-800" role="alert">
            <div class="inline-flex items-center justify-center flex-shrink-0 w-8 h-8 text-red-500 bg-red-100 rounded-xl: dark:bg-red-800 dark:text-red-200">
                <svg class="w-5 h-5" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 20 20">
